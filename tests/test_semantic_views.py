...
def cand(source_id: str, *, text: str, predicate: str = "STATE", status: str = "current") -> SemanticCandidate:
    return SemanticCandidate(
        ...
        predicate=predicate,
        confidence="high",
        evidence_refs=[EvidenceRef("raw_entry", f"entry_{source_id}", "2026-09-21T10:00:00+00:00", "establishes")],
    )
...
class TestSemanticViews(unittest.TestCase):
    def test_current_status_view_prioritizes_state_sources_and_confidence(self):
        situation = assemble_situation(
            ...
            candidates=[cand("state", text="Project Atlas is read-only"), cand("decision", text="Decision: no external service", predicate="DECISION")],
        )
    ...
    def test_decision_rationale_view_prioritizes_decisions_before_evidence(self):
        situation = assemble_situation(
            ...
            candidates=[cand("state", text="Project Atlas is read-only"), cand("decision", text="Decision: keep prototype offline", predicate="DECISION")],
        )
        ...
        self.assertIn("Decision: keep prototype offline", rendered)
    ...
    def test_next_action_view_prioritizes_open_questions_and_blockers(self):
        situation = assemble_situation(
            topic="Project Atlas",
            purpose="next_action",
            candidates=[cand("open", text="Open question: verify whether stale state is suppressed", predicate="OPEN_QUESTION"), cand("state", text="Project Atlas is read-only")],
        )
        ...
        self.assertIn("Open question: verify whether stale state is suppressed", rendered)
